...
import br.edu.ufcg.analytics.diferentonas.util.TextAnalyzer;
import scala.Tuple2;
...
public class SimilarityBatchJobSpark extends AbstractSparkBatchJob {
	...
	public void run(String... args) throws IOException {
		...
		int inputLines = Integer.valueOf(args[3]);
		...
		JavaRDD<Row> rows = javaFunctions(getJavaSparkContext())
				...
				.mapToPair( row -> new Tuple2<>(row.getInt(keyColumnName), row.getString(valueColumnName)))
				.mapValues( value -> TextAnalyzer.filter(value))
				.map(tuple -> RowFactory.create(tuple._1, tuple._2));
		
		rows = getJavaSparkContext().parallelize(rows.takeSample(false, inputLines));
